...
class BulkImportItem(BaseModel):
    ...
    id: Optional[str] = None         # The temporary React ID
    parent_id: Optional[str] = None  # Temporary React parent ID
    folder_id: Optional[str] = None  # Temporary React folder ID
    name: Optional[str] = None       # For folders
    title: Optional[str] = None      # For bookmarks
    url: Optional[str] = None        # For bookmarks
    archive: Optional[bool] = False  # For bookmarks
...
@app.post("/import")
def bulk_import(req: BulkImportRequest, background_tasks: BackgroundTasks): # <--- ADDED HERE
    if not req.items:
    ...
    # ==========================================
    # PASS 3: TRIGGER BACKGROUND JOBS
    # ==========================================
    if archived_bids_to_fetch:
        ...
        # SWAPPED THIS LINE! FastAPI safely hands this to the event loop after responding.
        background_tasks.add_task(fetch_worker, job_id, archived_bids_to_fetch, True, True)

    return {
        "status": "success", 
        "imported": imported_count, 
        "background_jobs_started": len(archived_bids_to_fetch)
    }